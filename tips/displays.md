...
Not all reviewers provide T0-100% response time measurements, but **they should be**. If you're interested to see if a reviewer is properly testing response times, you can compare their data against data from an outlet like [RTINGS.com](https://RTINGS.com) or reputable YouTube reviewers like [Jarrod's Tech](https://www.youtube.com/jarrodstech) and [Hardware Unboxed](https://www.youtube.com/@Hardwareunboxed). Response time numbers are rarely less than 1ms except on extremely high refresh rate panels. Either that, or you're looking an an OLED screen.
...
It's worth noting that some display technologies, like OLEDs, don't have "traditional" response times. Instead, these displays sometimes suffer from effects like black smearing when content moves on pure black backgrounds. This can be tested for, although most reviewers don't check response times on OLEDS due to difficulties associated with OLEDs running on PWM. This causes response time data to appear very noisy, making precise measurements difficult.